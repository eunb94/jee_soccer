...
SELECT PLAYER_NAME 선수명, 
       CONCAT(HEIGHT IS NULL,HEIGHT, 'cm')키
       CONCAT(WHEN WEIGHT IS NULL, WEIGHT, 'kg')몸무게
FROM PLAYER
WHERE TEAM_ID LIKE 'K02'
ORDER BY HEIGHT DESC
; 
...
SELECT T.TEAM_NAME 팀명, P.PLAYER_NAME 선수이름, P.POSITION 포지션 
...
SELECT T.TEAM_NAME 팀명, P.PLAYER_NAME 선수이름, P.HEIGHT 키
FROM PLAYER P JOIN TEAM T
    ON P.TEAM_ID LIKE T.TEAM_ID
WHERE T.TEAM_ID IN ('K02', 'K10')
...
SELECT T.TEAM_NAME 팀명, P.PLAYER_NAME 선수이름
FROM PLAYER P JOIN TEAM T
    ON P.TEAM_ID LIKE T.TEAM_ID
...
SELECT T.TEAM_NAME 팀이름, S.STADIUM_NAME 스타디움이름
...
SELECT T.TEAM_NAME 팀이름, S.STADIUM_NAME 스타디움이름, C.AWAYTEAM_ID 원정팀ID, C.SCHE_DATE 날짜
FROM TEAM T 
    JOIN STADIUM S
        ON(T.STADIUM_ID LIKE S.STADIUM_ID)
    JOIN SCHEDULE C
...
SELECT P.PLAYER_NAME 선수이름, P.POSITION 포지션, '포항 '||T.TEAM_NAME 팀이름, S.STADIUM_NAME 스타디움이름, C.SCHE_DATE 경기날짜
FROM TEAM T 
    JOIN PLAYER P
...
SELECT S.STADIUM_NAME 스타디움, C.SCHE_DATE 경기일정, C.HOMETEAM_ID 홈팀, C.AWAYTEAM_ID 원정팀, C.HOME_SCORE "홈팀 점수", C.AWAY_SCORE "원정팀 점수"
...
SELECT S.STADIUM_NAME 스타디움, S.STADIUM_ID 코드, S.SEAT_COUNT 좌석수, S.HOMETEAM_ID 홈팀, T.E_TEAM_NAME
...
HAVING AVG(P.HEIGHT) < (SELECT ROUND(AVG(HEIGHT),2)평균키
                        FROM PLAYER
...
FROM TEAM T JOIN PLAYER P
    ON T.TEAM_ID LIKE P.TEAM_ID
WHERE P.HEIGHT < (SELECT AVG(O.HEIGHT)
                  FROM PLAYER O
                  WHERE P.TEAM_ID LIKE O.TEAM_ID
...
SELECT S.STADIUM_ID ID, S.STADIUM_NAME 스타디움
FROM STADIUM S
WHERE EXISTS (SELECT STADIUM_ID
              FROM SCHEDULE C
...
SELECT 
    ROUND(AVG(CASE WHEN POSITION LIKE 'FW' THEN HEIGHT END), 2) 포워더,
...
SELECT TEAM_ID 팀아이디, T.TEAM_NAME 팀명, P.PLAYER_NAME 선수, P.POSITION 포지션, P.BACK_NO 백넘버, P.HEIGHT 키 
FROM TEAM T JOIN PLAYER P
        USING (TEAM_ID)
...
SELECT TEAM_ID 팀아이디, TEAM_NAME 팀명, PLAYER_NAME 선수, POSITION 포지션, BACK_NO 백넘버, HEIGHT 키 
...
SELECT C.SCHE_DATE, 
    (SELECT TEAM_NAME
     ...
     WHERE TEAM_ID LIKE C.HOMETEAM_ID)||'-'||
        ...
        C.HOME_SCORE||'-'||C.AWAY_SCORE 점수, 
        ABS(C.HOME_SCORE-C.AWAY_SCORE) 점수차
FROM SCHEDULE C
        JOIN TEAM T
            USING (STADIUM_ID)
        JOIN STADIUM S
...
FROM (SELECT C.SCHE_DATE, T.TEAM_NAME, C.HOME_SCORE, C.AWAY_SCORE, C.HOMETEAM_ID, C.AWAYTEAM_ID
      FROM SCHEDULE C
        ...
        JOIN STADIUM S
            USING (STADIUM_ID)
      WHERE SCHE_DATE LIKE '201208%')S;

-- SOCCER_SQL_026 
-- 20120501 부터 20120602 사이에 경기가 있는 경기장 조회

SELECT DISTINCT STADIUM_NAME 스타디움, STADIUM_ID 코드
FROM STADIUM 
        JOIN SCHEDULE 
            USING(STADIUM_ID)
...
SELECT DISTINCT STADIUM_NAME 스타디움, 
                STADIUM_ID 코드
...
SELECT T.TEAM_NAME 팀명, 
        P.PLAYER_NAME 선수명, 
        P.HEIGHT 키, 
       (SELECT ROUND(AVG(O.HEIGHT), 2)
        FROM PLAYER O
        WHERE TEAM_ID LIKE TEAM_ID)평균키
FROM TEAM T 
        JOIN PLAYER P
            USING(TEAM_ID)
ORDER BY 4 DESC
...
SELECT TEAM_ID 팀아이디, T.TEAM_NAME 팀명, ROUND(AVG(P.HEIGHT), 2)평균키
FROM TEAM T 
        JOIN PLAYER P
            USING(TEAM_ID)
...
SELECT COUNT(*)
FROM PLAYER
WHERE TEAM_ID NOT IN ('K07', 'K08', 'K09')
